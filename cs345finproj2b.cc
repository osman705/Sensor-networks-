...
double distance_between_node0_node1 = 50; //y-axis distance
double node1Velocity = 130; //node 1 velocity in x-axis (m/s)
...
//      NS_LOG_UNCOND(Simulator::Now ().GetSeconds () << " Node 1 received a packet. UNIFIED receiver.");
        ...
        if (type_of_packet == 1) //data
        {
                if (Pkt_no_last_seen_by_node1 < Pkt_no_being_sent_by_node0)
                {
                        ...
                        if ( Pkt_no_last_seen_by_node1 == 0 ) {     // first packet that arrives for node 1
