...
uint16_t node2_head = 0;
uint16_t node2_tail = 0;
...
double distance_between_node1_node3 = -250; //x-axis distance
double distance_between_node0_node1 = 3; //y-axis distance
double node1_velocity = 20; //node 1 velocity in x-axis (m/s)
...
//      NS_LOG_UNCOND(Simulator::Now ().GetSeconds () << " Node 1 received a packet. UNIFIED receiver.");
        ...
        if (type_of_packet == 1) //data
        {
                if (Pkt_no_last_seen_by_node1 < Pkt_no_being_sent_by_node0)
                {
                        ...
                        if ( Pkt_no_last_seen_by_node1 == 0 ) {     //first packet received by node 1
...
static void Node2ReceivePacket (Ptr<OutputStreamWrapper> stream, Ptr<Socket> socket)
{
        ...
        {
                if (Pkt_no_last_seen_by_node2 == 0)
                {
                        ...
                        std::cout << "Node2 countMail: " << countMail << std::endl;
                        
                        node2_head = receivedpacket_head;
                        node2_tail = receivedpacket_tail;
                        Node2SendAck = 1;
                }
                else
                {
                        if (receivedpacket_tail > node2_head)
                        {
                                Pkt_no_last_seen_by_node2 = receivedpacket_globalcounter;
                                NS_LOG_UNCOND(Simulator::Now ().GetSeconds () << " Node 2 received a packet.");
                                std::cout << "*------------*" <<  std::endl;
                        ...
                        else
                        {
                                ...
                                newMails = receivedpacket_head - node2_head;
                                countMail = countMail + newMails;
...
{
//      NS_LOG_UNCOND(Simulator::Now ().GetSeconds () << " Node 3 received a packet. UNIFIED receiver.");
        ...
        if (type_of_packet == 1) //data
        {
                if (Pkt_no_last_seen_by_node3 < Pkt_no_being_sent_by_node0)
                {
                        ...
                        if ( Pkt_no_last_seen_by_node3 == 0 ) {     //first packet received by node 3
                                node3_head = header.Gethead ();
                                node3_tail = header.Gettail ();
                        }
                        else {
                                node3_head = header.Gethead ();
                        }
...
//---Higher level code for node 3 END
...
int main (int argc, char *argv[])
{
        ...
        adhocNodes.Create(4);  //All 3 nodes are members of adhocNodes
        NodeContainer mobileNode = NodeContainer (adhocNodes.Get(1), adhocNodes.Get(3)); //Node 1 and Node 3 classed as mobile nodes
        ...
        // 5. Set up mobility model for Node 1 and Node 3
        ...
        positionAllocMobileNode->Add (Vector (distance_between_node1_node3, distance_between_node0_node1, 0.0));
        ...
        Ptr<ConstantVelocityMobilityModel> mob_1 = mobileNode.Get(0)->GetObject<ConstantVelocityMobilityModel>();
        mob_1->SetVelocity(Vector(node1_velocity, 0, 0)); //velocity 20 m/s in x-axis
	    Ptr<ConstantVelocityMobilityModel> mob_3 = mobileNode.Get(1)->GetObject<ConstantVelocityMobilityModel>();
        mob_3->SetVelocity(Vector(node1_velocity, 0, 0)); //velocity 20 m/s in x-axis
        ...
        Ptr<OutputStreamWrapper> locationStream = asciiTraceHelper.CreateFileStream ("Location1.txt");
        ...
        Ptr<Socket> Node3Recv = Socket::CreateSocket (adhocNodes.Get (3), tid1);
	...
	//Start node 2's ACK loop (node 2---> node 3, socket 9)
	//Fire up Node2AckLoop
        ...
        sourceSocketforNode2SECOND->Connect (SocketAddressof3);
